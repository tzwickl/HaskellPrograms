...
data State = State (M.Map Position Disc) MyColor
...
instance Eq State where
	(==) (State map1 myColor1) (State map2 myColor2) = M.elems map1 == M.elems map2 && myColor1 == myColor2
...
startState whoAmI = setInitialGameBoard whoAmI
...
nextState (oldState@(State map oCol)) Pass = calculateBestMove (Disc oCol) oldState
nextState (oldState@(State map oCol)) move 
				| oldState == newState = (Pass, oldState)
				| otherwise 		   = calculateBestMove (Disc oCol) newState where
						newState = processMove move mCol oldState
...
initializeGameBoard :: MyColor -> State
initializeGameBoard mCol = State (ins M.empty [((x,y), Empty)| x <- [0..7], y <- [0..7]]) mCol where
		ins map []				= map
...
setInitialGameBoard mCol = upd state [((3,3), Disc White), ((4,4), Disc White), 
		...
		state		   				= initializeGameBoard mCol
		upd   st [] 				= st
...
processMove :: Move -> Disc -> State -> State
processMove m col st 
		| not $ null $ flipDiscs = flipAllDiscs flipDiscs (changeColOnPos pos col st)
		| otherwise  = st where
			Play pos  = m
...
changeColOnPos pos col st = State (M.adjust (\x -> col) pos map) mCol where
		State map mCol = st
...
getColorOnPos pos state = fromJust $ M.lookup pos map where
		State map _ = state
		fromJust (Just x) = x
...
calculateBestMove col st 
	| null allPossibleMoves = (Pass, st)
	| otherwise 			= (bestMove, newState) where
		newState			= processMove bestMove col st
		bestMove			= Play $ allPossibleMoves !! indexOfMax
		(Just indexOfMax)	= DL.elemIndex (maximum advantages) advantages
		allPossibleMoves 	= getAllLegalPos col st
		advantages			= map (\pos -> minMaxTree 4 col st) allPossibleMoves
...
numPiecesAdvantage mCol (State map _) = count $ M.elems map where
		oCol = getOpponentCol mCol
		count [] = 0
		count (x:xs) 
			| x == mCol = 1  + count xs
...
-- simulates n moves alternating between the two adversaries and calculates the maximal/minimal advantage
-- for each move according to the heuristic table and the advantage
-- n determines how many moves the algorithm should look ahead before terminating
-- after n steps the max value for this move is returned
minMaxTree :: Int -> Disc -> State -> Int
minMaxTree n (col@(Disc c)) (st@(State _ mCol)) 
			| null allPossibleMovesOp || null allPossibleMovesMe = if c == mCol then 1000 else -1000
			| n == 0      =   advantage col st
			| c == mCol   =   adv + advantage col st
			| otherwise   = -(adv + advantage col st) where
				oCol 				= getOpponentCol col
				allPossibleMovesOp  = getAllLegalPos oCol st
				allPossibleMovesMe  = getAllLegalPos col st
				adv 				= maximum $ map (\pos -> minMaxTree (n-1) oCol (processMove (Play pos) col st) + 
											getWeight pos) allPossibleMovesMe
